...
| 接入器 | 对外暴露 | 载荷载体 |
|--------|----------|----------|
| `LabAcacia.McpIngress` | MCP 2024-11-05 服务端：`tools/list` / `tools/call` | CapsFrame 序列化后塞进 `content[{type:"text", text:"..."}]` |
| `LabAcacia.A2aIngress` | Google A2A v0.2 服务端：`tasks/send` / `tasks/get` | CapsFrame 内联为 `artifacts[].parts[{type:"data", data:…}]` |
| `LabAcacia.GrpcIngress` | gRPC 服务 `NwpIngress.Invoke`（本演示跑 h2c） | CapsFrame 序列化成 JSON → 挂进 `bytes body_json` |
...
**场景 C —— 走 gRPC。** `NwpIngress.Invoke` 跑在 h2c 上：
...
需要 .NET 10 SDK。4 个 Kestrel host（上游 + 3 个接入器）和客户端全部在一个
...
├── HostBuilders.cs                   # Kestrel 工厂：上游 + 3 个接入适配器
├── GreetingsProvider.cs              # greetings.hello 的 IActionNodeProvider 实现
├── Protos/nwp_bridge_client.proto    # 接入器 proto 的本地拷贝（仅 Client）
...
`LabAcacia.GrpcIngress` 发出去的 proto 是 `GrpcServices="Server"`，只生成
服务端基类。演示需要**客户端 stub**，同时又引用了接入库作为服务端——如果
从同一份 proto 同时生成两侧 stub，就会在
`LabAcacia.GrpcIngress.Generated.*` 下撞消息类型名。解决方式是把 proto
本地复制一份，改成 `csharp_namespace = "NPS.Demo.IngressPlayground.Grpc"` +
...
- **仅单个动作。** 演示只打了一个动作。MCP `resources/*`（对 Memory Node
  ...
  ——要看完整面，请参考 `compat/{mcp,a2a,grpc}-ingress/` 的单测。
...
- **运行结果为重命名前的记录。** 上方截获的输出是 `bridge-playground` →
  `ingress-playground` 重命名之前记录的；`anchor_ref` URL 仍显示
  `bridge-playground`。新运行后会显示 `ingress-playground`。
